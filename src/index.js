...
app.get("/api/enrollments", checkAuth, async (req, res) => {
  try {
    ...
    res.json({
      status: "success",
      message: "Enrollments berhasil diambil",
      count: rows.length,
      data: rows,
    });
  } catch (err) {
  ...
  try {
    ...
    if (!student_id || !course_id) {
      return res
        ...
        .json({
          status: "error",
          message: "student_id dan course_id wajib diisi",
        });
    }
    ...
    if (requesterRole === "murid" && requesterId !== student_id) {
      return res.status(403).json({
        status: "error",
        message:
          "Akses ditolak: Murid hanya bisa mendaftarkan dirinya sendiri.",
      });
    }
    ...
    try {
      ...
      const courseRes = await axios.get(
        `${COURSE_SERVICE_URL}/api/courses/${course_id}`,
        {
      ...
      if (
        courseRes.data.status !== "success" ||
        !courseRes.data.data.schedule
      ) {
        return res
          .status(404)
          .json({
            status: "error",
            message: "Kursus tidak ditemukan atau belum punya jadwal.",
          });
      }
      newCourse = courseRes.data.data;
    } catch (err) {
      return res
        .status(500)
        .json({
          status: "error",
          message: "Gagal menghubungi Course Service",
          error: err.message,
        });
    }
    ...
    for (const row of studentEnrollments) {
      ...
      try {
        const token = req.headers["authorization"];
        const courseRes = await axios.get(
          `${COURSE_SERVICE_URL}/api/courses/${currentCourseId}`,
          {
            headers: { Authorization: token, Accept: "application/json" },
          }
        );
        if (
          ...
          continue;
        currentCourseSchedule = courseRes.data.data.schedule;
      } catch (err) {
        continue;
      }
      ...
      if (checkClash(currentCourseSchedule, newCourse.schedule)) {
        return res.status(409).json({
          status: "error",
          message: `Jadwal kursus bentrok dengan Course ID: ${currentCourseId}`,
        });
      }
    }
    ...
    if (exists.length > 0) {
      return res
        ...
        .json({
          status: "error",
          message: "Siswa sudah terdaftar dalam kursus ini",
        });
    }
    ...
    res
      ...
      .json({
        status: "success",
        message: "Siswa berhasil didaftarkan ke kursus",
      });
  } catch (err) {
    res.status(500).json({ status: "error", message: err.message });
  }
...
app.get("/api", (req, res) => {
  res.json({
    status: "success",
    message: "Enrollment Service API is running!",
  });
});
